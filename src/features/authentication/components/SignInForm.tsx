  ...
  const onSubmit = useCallback(
    async (data: FormData) => {
      try {
        ...
        identifierForm.playSuccessProgression(); // Or a general success sound
      } catch (err) {
        // Error is handled by the useAuthActions hook and displayed below
        identifierForm.playFailureProgression(); // Or a general failure sound
        console.error('Sign in failed:', err); // Log error for debugging
      }
    },
    [
      mode,
