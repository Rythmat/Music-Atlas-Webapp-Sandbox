...
import { SelectNoteSequenceModal } from '@/features/admin/noteSequences/components/SelectNoteSequenceModal';
...
export const PageEditorPane = forwardRef<HTMLDivElement, PageEditorPaneProps>(
  (
    ...
    const handleInsertSequence = (sequenceId: string) => {
      if (!textareaRef.current) return;

      const textarea = textareaRef.current;
      const { selectionStart } = textarea;

      // Insert the sequence component at the cursor position
      const newContent =
        content.substring(0, selectionStart) +
        `_component:sequence(${sequenceId}, piano)_` +
